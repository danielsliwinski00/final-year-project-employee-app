...
export default class ViewOrders extends Component {
    ...
    getOrders() {
        return fetch("http://192.168.1.209:8080/api/vieworders.php",
            {
    ...
    orderComplete() {
        return fetch("http://192.168.1.209:8080/api/ordercomplete.php",
            {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    'orderTime': this.state.orderTime,
                    'orderTableNumber': this.state.orderTableNumber
                })
            })
            .then((response) => {
                return response.json();
                    ...
                    <TouchableOpacity
                        ...
                        <View style={[styles.optionsPanelContacts, { height: 160, width: '90%', top: 200, right: '5%', borderRadius: 15 }]}>
                            <TouchableOpacity
                                ...
                                onPress={() => { this.setState({ orderCompleteModal: false }), this.props.navigation.navigate('EditDishes', { dishID: this.state.dishID }) }}>
